...
namespace WebColegioPagos.Services
{
    public class ApiService : IApiService
    {
        ...
        public async Task<bool> loginAdmin(Admin admin)
        {
            ...
            {
                login = JsonConvert.DeserializeObject<bool>(await respuesta.Content.ReadAsStringAsync());
                return login;
            }
        ...
        {
            var respuesta = await _httpClient.PutAsJsonAsync<Estudiante>($"Estudiante/resetearContrasenia/{id}", null);
            if (respuesta.IsSuccessStatusCode)
            {
                return true;
            }
